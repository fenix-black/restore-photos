...
const ImageDisplay: React.FC<{ label: string; imageSrc: string | null; isLoading?: boolean; isDownloadable?: boolean; onDownload?: () => void; }> = ({ label, imageSrc, isLoading = false, isDownloadable, onDownload }) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center">
      ...
      {isDownloadable && imageSrc && !isLoading && (
          <button
              onClick={onDownload}
              className="mt-4 flex items-center justify-center gap-2 px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white font-semibold rounded-lg shadow-md transition-transform transform hover:scale-105"
          >
...
const ImageRestorationStage: React.FC<ImageRestorationStageProps> = ({ title, originalImage, processedImage, isLoading, isDownloadable, onDownload }) => {
  const { t } = useLocalization();
  return (
    <section className="w-full p-6 bg-brand-dark/50 rounded-2xl animate-slide-in-up">
      ...
      <div className="flex flex-col sm:flex-row gap-6">
        ...
        <ImageDisplay label={t('labelAfter')} imageSrc={processedImage} isLoading={isLoading} isDownloadable={isDownloadable} onDownload={onDownload} />
      </div>
    </section>
  );
};
